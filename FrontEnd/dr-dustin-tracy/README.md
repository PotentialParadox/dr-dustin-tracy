...
* [`src/dr_dustin_tracy/`](src/dr_dustin_tracy/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/dr_dustin_tracy/core.cljs): contains the SPA entry point, `init`
* [`test/dr_dustin_tracy/`](test/dr_dustin_tracy/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `dr-dustin-tracy` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/dr_dustin_tracy/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns dr-dustin-tracy.example
  (:require [dr-dustin-tracy.config :as config])
